...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Overlay
    className={cn(
      'fixed inset-0 z-50 bg-overlay-transparent backdrop-blur-sm data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0',
      className,
    )}
    {...props}
    ref={ref}
...
const sheetVariants = cva(
  'fixed z-50 gap-4 bg-foreground p-6 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500',
  {
    variants: {
      ...
      variant: {
        primary: 'bg-glass backdrop-blur-lg',
        secondary: 'bg-ciGray',
      },
    },
    defaultVariants: {
    ...
    <SheetPortal>
      ...
      <SheetPrimitive.Content
        ...
        className={cn(
          ...
          { 'bg-accent text-background': variant === 'secondary' },
        ...
        {showCloseButton && (
          <SheetPrimitive.Close
            className={cn(
              ...
              'absolute right-4 top-6 flex h-8 items-center justify-center rounded-sm opacity-70 ring-offset-background transition-opacity data-[state=open]:bg-secondary hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none',
            )}
          >
            <FontAwesomeIcon
              icon={faClose}
